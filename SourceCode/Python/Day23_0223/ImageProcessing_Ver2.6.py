...
def functionMenu():
    ...
    edgeDetectMenu.add_cascade(label='1. 이동과 차분 엣지', menu=shiftDiffer)
...
def AIfunctionMenu():
    ...
    WebCam_aiImageProcessing = Menu(haarRecog)

    aiMenu.add_command(label='입력키 리턴값 확인', command=checkRetKey)
    aiMenu.add_separator()
    aiMenu.add_cascade(label='1. 이미지 속 객체 인식 (Haar)', menu=haarRecog)
    haarRecog.add_cascade(label='1. 사람 인식', menu=haarHumanRecog)
    ...
    haarRecog.add_command(label='2. 자동차 인식', command=aiCarImage)
    haarRecog.add_cascade(label='3. 사진 얼굴 영상처리', menu=aiImageProcessing)
    ...
    haarRecog.add_cascade(label='4. 웹캠 영상처리', menu=WebCam_aiImageProcessing)
    ...
    aiVideoMenu.add_command(label='1. 이미지 속 객체 인식 (SSD)', command=DLImage_Photo)
    #aiVideoMenu.add_command(label='2. 동영상 속 객체 인식 (SSD)', command=DLImage_Video)

    # 웹캠 메뉴_AI
    WebCamMenu = Menu(aiMenu)  # 상위 메뉴 (파일)

    aiMenu.add_cascade(label='3. 웹캠', menu=WebCamMenu)
    WebCamMenu.add_command(label='1. 웹캠', command=WebCamDisplay_Threading)
    WebCamMenu.add_command(label='2. 웹캠_객체 인식 (Haar)', command=haar_WebCamDisplay_Threading)
    WebCamMenu.add_command(label='3. 웹캠_객체 인식 (SSD)', command=SSD_WebCamDisplay_Threading)

    # 유튜브 메뉴_AI
    YoutubeMenu = Menu(aiMenu)  # 상위 메뉴 (파일)

    aiMenu.add_cascade(label='4. 유튜브', menu=YoutubeMenu)
    YoutubeMenu.add_command(label='1. 유튜브', command=YouTubeDisplay_Threading)
    YoutubeMenu.add_command(label='2. 유튜브_객체 인식 (Haar)', command=None)
    YoutubeMenu.add_command(label='3. 유튜브_객체 인식 (SSD)', command=None)
    ...
    while True:
        ...
        for (x, y, w, h) in face_rects:
            ...
            # 얼굴 카툰화 (Haar)_AI
                ...
                # medi = cv2.medianBlur(gray, 7)
                # edges = cv2.Laplacian(medi, cv2.CV_8U, ksize=5)
                # ret, mask = cv2.threshold(edges, 100, 255, cv2.THRESH_BINARY_INV)
                # frame[y:y + h, x:x + w] = cv2.cvtColor(crop, cv2.COLOR_GRAY2RGB)
