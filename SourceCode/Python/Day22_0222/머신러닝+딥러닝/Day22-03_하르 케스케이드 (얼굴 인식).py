# Day22-03_하르 케스케이드 (얼굴 인식).py (Color Image Processing with OpenCV & 인공지능)
...
def cvGammaImage():
    ...
    ## ***** 영상처리 알고리즘 ***** ##
    gammaValue = 0.0
    gammaValue = askfloat("감마 변환 처리", "감마 변환 값 입력 : ", minvalue=0, maxvalue=255)

    print("[OpenCV] 감마 변환 이미지, 변환값 : %.1f" % gammaValue)

    ### openCV용 함수 활용 ###
    outPhoto = outPhoto.astpye(np.float)
...
# 얼굴 추출
def aiFaceImage():
    global inPhoto, outPhoto, anoPhoto
    global inImage, outImage, anoImage, inH, inW, outH, outW
    global window, canvas, paper
    global sx, sy, ex, ey
...
# *************************** < 메인 함수 > ***************************
window = Tk()
window.title(" GrayScale Image Processing Ver2.6 (인공지능_하르 케스케이드)")
...
## OpenCV용 메뉴 ##
openCVMenu = Menu(mainMenu)       # 상위 메뉴 (OpenCV)
...
openCVMenu.add_cascade(label='1. 화소점 처리_CV', menu=CVpxDotMenu)
...
# 2. 히스토그램 처리_OpenCV
CVhistoMenu = Menu(funcMenu)       # 상위 메뉴 (파일)

openCVMenu.add_cascade(label='2. 히스토그램 처리_CV', menu=CVhistoMenu)
...
# 3. 화소 영역 처리
CVpxAreaMenu = Menu(funcMenu)     # 상위 메뉴 (파일)
CVblurring = Menu(pxAreaMenu)         # 블러링
CVSharpening = Menu(pxAreaMenu)        # 샤프닝

openCVMenu.add_cascade(label='3. 화소 영역 처리_CV', menu=CVpxAreaMenu)
...
aiMenu.add_command(label='얼굴 추출', command=aiFaceImage)
aiMenu.add_command(label='눈 추출', command=aiEyesImage)
aiMenu.add_command(label='코 추출', command=aiNoseImage)
