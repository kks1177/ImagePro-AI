...
int choiceMenu() {
	...
	Sleep(1000);		system("cls");	 mainMenu();
...
void openImage() {
	...
	unsigned long fsize = ftell(rfp);			// 파일 위치    262144(=512x512)
	fclose(rfp);

	inH = inW = sqrt(fsize);			// (아주 중요!) 픽셀 수 파악하는 코드
	// 입력영상 (inImage)의 메모리 동적 할당
...
void ori_printScreen() {
	if (pix == 1)			inImg_128();		// Original 128x128
	else if (pix == 2)	inImg_256();		// Original 256x256
	else if (pix == 3)	inImg_512();		// Original 512x512
	else
		printf(" 파일 업로드 오류! \n");
}
// 결과 이미지 화면 출력
void res_printScreen() {
	...
	/*
	if (pix == 1)			outImg_128();		// 128x128
	else if (pix == 2)	outImg_256();		// 256x256
	else if (pix == 3)	outImg_512();		// 512x512
	*/
}
//// ********************************************************************************* ////
...
void clearControlImage() {
	...
	switch (inKey) {
	case '1':	clearUp(value);			break;
	case '2':	clearDown(value);		break;
	default:
...
void grayScaleImage() {
	...
	inKey = _getch();
	switch (inKey) {
	case '1':	grayScale128();		break;
	case '2':	grayScaleAvg();		break;
	default:
		printf("다시 입력하세요! \n");
		break;
	}
...
void bitCalcImage() {
	...
	switch (inKey) {
	case '1':	bitCalc_And(value);		break;
	case '2':	bitCalc_Or(value);			break;
	case '3':	bitCalc_Xor(value);			break;
	case '4':	bitCalc_Not();				break;
	default:
		printf("다시 입력하세요! \n");
		break;
	}
	...
	if (value != 0)
		for (int i = 0; i < inH; i++)
			for (int k = 0; k < inW; k++) {
				...
				if (gamma > 255)		outImage[i][k] = 255;
				else if (gamma < 0)	outImage[i][k] = 0;
				else							outImage[i][k] = gamma;
			}
	else
		printf(" 다시 입력하세요! \n");
...
void parabolaImage() {
	...
	inKey = _getch();
	switch (inKey) {
	case '1':	parabolaCap();		break;
	case '2':	parabolaCup();		break;
	default:
		printf("\t\t 다시 입력하세요! \n");
		break;
	}
...
void stretchedImage() {
	...
	for (int i = 0; i < inH; i++) {
		for (int k = 0; k < inW; k++) {
			...
			if (px > 255)		outImage[i][k] = 255;
			else if (px < 0)	outImage[i][k] = 0;
			else					outImage[i][k] = (unsigned char)px;
		}
	}
	printf("\t\t\t\t\t  < 스트래칭 화면 > ");
	res_printScreen();
...
void printHisto() {
	...
	// 정규화된 히스토그램의 값은 출력배열에 검은점(0)으로 표현
	for (int i = 0; i < outH; i++) 
...
void mirroringImage() {
	...
	inKey = _getch();
	switch (inKey) {
	case '1':	L_R_mirroring();		break;
	case '2':	T_B_mirroring();		break;
	case '3':	TB_LR_mirroring();	break;
	default:
		printf(" 다시 입력하세요! \n");
		break;
	}
...
void turnImage() {
	...
	if (direct == 1) {
		...
		switch (inKey) {
		case '1':	turn90Right();	break;
		case '2':	turn180Right();	break;
		case '3':	turn270Right();	break;
		default:
			break;
		}
	}
	// 반시계방향 회전
		...
		switch (inKey) {
		case '1':	turn90Left();		break;
		case '2':	turn180Left();	break;
		case '3':	turn270Left();	break;
		default:
			break;
		}
	}
	// 자유회전
		...
		inKey = _getch();
		switch (inKey) {
		case '1':	turnFree(angle);			break;
		case '2':	turnFreeCenter(angle);	break;
		default:
			printf("\n\t\t 입력 오류! 재입력 바랍니다! \n");
			Sleep(1000);	system("cls");
			mainMenu();	printf("\t\t 파일명 --> ");
...
void TB_LR_mirroring() {
	for (int i = 0; i < inW; i++) {
		for (int k = 0; k < inH; k++) {
			int mirror_i = inH - 1 - i;			// (최대행 - 1) - 반복자
			int mirror_k = inW - 1 - k;		// (최대열 - 1) - 반복자
...
void turn90Right() {
	for (int i = 0; i < inW; i++) 
		for (int k = 0; k < inH; k++) 
			outImage[i][k] = inImage[inH - k - 1][i];			// 최대행 - 반복자 - 1
...
void turn180Right() {
	for (int i = 0; i < inW; i++) 
		for (int k = 0; k < inH; k++) 
			outImage[i][k] = inImage[inH - i - 1][inW - k - 1];		// 최대행 - 반복자 - 1			// 최대열 - 반복자 - 1
...
void turn270Right() {
	for (int i = 0; i < inW; i++) 
		for (int k = 0; k < inH; k++) 
			outImage[i][k] = inImage[k][inH - i - 1];			// 최대행 - 반복자 - 1
...
void turn90Left() {
	for (int i = 0; i < inH; i++) 
		for (int k = 0; k < inW; k++) 
			outImage[i][k] = inImage[k][inH - 1 - i];		// 최대행 - 1 - 반복자
...
void turn180Left() {
	for (int i = 0; i < inH; i++) 
		for (int k = 0; k < inW; k++)
			outImage[i][k] = inImage[inH - 1 - i][inW - k - 1];		// 최대행 - 1 - 반복자			// 최대열 - 반복자 - 1
