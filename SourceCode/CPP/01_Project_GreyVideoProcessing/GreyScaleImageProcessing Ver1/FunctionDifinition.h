...
void printMenu() {
	puts("\n");
	puts("\t\t\t      ===========================================================================");
	puts("\t\t\t                     ### GreyScale Video Processing Ver 1.0_김기성 ###\n");
	puts("\t\t\t      ---------------------------------------------------------------------------");
	puts("\t\t\t\t [ 파일 메뉴 선택 ] \t 1.이미지 열기   2.저장   0.종료 \n\n");

	puts("\t\t\t\t [ 1.화소점 처리 기능 메뉴 선택 ] ");
	puts("\t\t\t\t     A.밝기  B.선명도  C.반전  D.흑백  E.비트연산 ");
	puts("\t\t\t\t     F.감마변환  G.범위강조  H.파라볼라 변환  I. 명암대비 스트래칭 \n");

	puts("\t\t\t\t [ 2.히스토그램 처리 기능 메뉴 선택 ] ");
	puts("\t\t\t\t     J.히스토그램 출력  K.스트래칭  L.엔드-인 탐색  M.평활화 \n");

	puts("\t\t\t\t [ 3.화소영역 처리 기능 메뉴 선택 ] ");
	puts("\t\t\t\t	 N.엠보싱  O.블러링  P.샤프닝  Q.스무딩 \n");

	puts("\t\t\t\t [ 4.기하학 처리 기능 메뉴 선택 ] ");
	puts("\t\t\t\t     R.미러링  S.회전  T.확대/축소  U.이동 \n");
	puts("\t\t\t      =========================================================================== \n");
...
void choiceMenu() {
	...
	while (inKey != '0') {
		...
		switch (inKey) {
		case '1':	system("cls");		printMenu();	openImage();	break;		// 파일 open
		case '2':	saveImage();	break;															// 파일 save
		...
		case 'J':	case 'j':		// 히스토그램 출력
			system("cls");	printMenu();	printHisto();	break;
		case 'K':	case 'k':		// 히스토그램 스트래칭
			system("cls");	printMenu();	histoStretchImage();	break;
		case 'L':	case 'l':	// 히스토그램 엔드-인 탐색
			system("cls");	printMenu();	histoEndInImage();	break;
		case 'M':	case 'm':		// 히스토그램 평활화
			system("cls");	printMenu();	histoEqualImage();	break;

		case 'N':	case 'n':	// 엠보싱
			system("cls");	printMenu();	embossingImage();	break;
		case 'O':	case 'o':	// 블러링
			system("cls");	printMenu();	blurringImage();	break;
		case 'P':	case 'p':	// 샤프닝
			system("cls");	printMenu();	sharpeningImage();	break;
		case 'Q':	case 'q':	// 스무딩 (가우시안)
			system("cls");	printMenu();	smoothingImage();	break;

		case 'R':	case 'r':	// 미러링 (좌우/상하/좌우+상하)
			system("cls");	printMenu();	mirroringImage();	break;
		case 'S':	case 's':	// 회전 (방향, 90/180/270도/자유)
			system("cls");	printMenu();	turnImage();	break;
		case 'T':	case 't':	// 이미지 크기(규격) 확대/축소
			system("cls");	printMenu();	imageSizeUpDown();	break;
		case 'U':	case 'u':	// 이미지 이동
...
// X.회전(오른쪽,왼쪽 90/180/270/자유-백워딩+중앙)
void turnImage() {
	free2D_out();
	...
	printf("\t\t [방향] 1.오른쪽  2.왼쪽  3.자유 ");
	scanf_s("%d", &direct);

	// 시계방향 회전
	if (direct == 1) {
		printf("\n\t\t\t\t\t\t\t\t    [각도_시계방향] A.90  B.180  C.270 ");
		inKey = _getch();
		switch (inKey) {
...
void inImg_128() {
	...
	for (int i = 0; i < inH; i++) {
		for (int k = 0; k < inW; k++) {
			px = inImage[i][k];
			SetPixel(hdc, k + 300, i + 550, RGB(px, px, px));		// hdc가 제공해주는 함수		// i, k -> k+50, i+250
		}
	}
}
// 이미지 파일 입력_256x256
void inImg_256() {
	...
	for (int i = 0; i < inH; i++) {
		for (int k = 0; k < inW; k++) {
			px = inImage[i][k];
			SetPixel(hdc, k + 250, i + 550, RGB(px, px, px));		// hdc가 제공해주는 함수		// i, k -> k+50, i+250
		}
	}
}
// 이미지 파일 입력_512x512
void inImg_512() {
	...
	for (int i = 0; i < inH; i++) {
		for (int k = 0; k < inW; k++) {
			px = inImage[i][k];
			SetPixel(hdc, k + 50, i + 550, RGB(px, px, px));		// hdc가 제공해주는 함수		// i, k -> k+50, i+250
		}
	}
}
// 이미지 파일 출력_128x128
void outImg_128() {
	...
	for (int i = 0; i < outH; i++) {
		for (int k = 0; k < outW; k++) {
			px = outImage[i][k];
			SetPixel(hdc, k + 800, i + 550, RGB(px, px, px));		// hdc가 제공해주는 함수		// i, k -> k+50, i+250
		}
	}
}
// 이미지 파일 출력_256x256
void outImg_256() {
	...
	for (int i = 0; i < outH; i++) {
		for (int k = 0; k < outW; k++) {
			px = outImage[i][k];
			SetPixel(hdc, k + 720, i + 550, RGB(px, px, px));		// hdc가 제공해주는 함수		// i, k -> k+50, i+250
		}
	}
}
// 이미지 파일 출력_512x512
void outImg_512() {
	...
	for (int i = 0; i < outH; i++) {
		for (int k = 0; k < outW; k++) {
			px = outImage[i][k];
			SetPixel(hdc, k + 600, i + 550, RGB(px, px, px));		// hdc가 제공해주는 함수		// i, k -> k+50, i+250
		}
	}
}
